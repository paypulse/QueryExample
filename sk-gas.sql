SELECT TOP 1 EVU_STEP_CD FROM EVU_STEP ST INNER JOIN EVU_STD S ON (ST.EVU_STD_ID = S.EVU_STD_ID) 
		WHERE S.EVU_YY = YEAR(GETDATE())
		AND CONVERT(CHAR(10), GETDATE(), 23) BETWEEN EVU_SDATE AND EVU_EDATE


SELECT * FROM [USER] 
		WHERE EMP_ID = '00812' OR EMP_NM = '00812'

SELECT TOP 1 
		CASE WHEN EMP_ID BETWEEN 8000 AND 9000 THEN '계약직'
		WHEN APPNT_CD = 1050 THEN '면수습'
		ELSE '실패' END AS APPNT_NM
		FROM USER_CAREER WHERE EMP_ID = '00812' ORDER BY STA_YMD DESC

SELECT EVU_EMP_AUTH AS EMP_AUTH FROM ADMIN_AUTH WHERE EVU_EMP_ID = '00812'
...
SELECT 
			...
			S.EVU_STEP_CD, S.EVU_STEP_NM, S.EVU_SDATE, S.EVU_EDATE, S.ORDER_NO, S.ETC_EVU_YN, S.EVU_OPEN_YN
...
/*selectMngCurEvuStep*/
		...
		FROM EVU_STEP ST INNER JOIN EVU_STD S ON (ST.EVU_STD_ID = S.EVU_STD_ID) 
		WHERE S.EVU_STD_ID = '202201'
...
/*나의 평가대상 상태별 카운트 selectEvuMngCnt2*/
...
SELECT NOTI_NO, NOTI_IMPORTANT, NOTI_SUBJECT, NOTI_CONTENT,
